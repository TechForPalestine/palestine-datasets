...
export const calcChecksum = (repoPath: string) => {
  const checksum = execSync(`shasum ${repoPath}`).toString().trim().split(" ")[0];
...
export const getChecksum = async (artifactKey: string, checksum: string) => {
  ...
  if (response.status === 404) {
    console.log(`getChecksum: no object exists for artifact key: ${artifactKey}`);
    return false;
  }
...
export const createArtifact = (artifactName: string, checksum: string) => {
  execSync(`tar -czf ${artifactName} --directory=site/src/generated/killed-in-gaza ./`);
...
export const downloadArtifact = async (
  ...
  if (artifactMatch) {
    ...
    console.warn("⚠️ checksum does not match available artifact, downloading anyway");
  }
