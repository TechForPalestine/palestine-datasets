...
import { bindHelpers } from "./svg-helpers-v2";
...
const render = async ({ mobile } = { mobile: false }) => {
  ...
  const componentFilePath = `site/src/generated/daily-chart-v2${mobile ? "-mobile" : ""}.tsx`;
  const componentJs = await transform(
    ...
    {
      componentName: `HomepageCasualtyChartV2${mobile ? "Mobile" : ""}`,
