...
| ext_massacres_cum       | same as `massacres_cum` but extrapolated ([see note](#breakdown-interval))                                      | no       |
...
| killed_children_cum     | cumulative number of children killed to the report date ([see note](#breakdown-interval))                       | yes      |
| killed_women_cum        | cumulative number of women killed to the report date ([see note](#breakdown-interval))                          | yes      |
...
| ext_killed_children_cum | same as `killed_children_cum` but extrapolated ([see note](#breakdown-interval))                                | no       |
| ext_killed_women_cum    | same as `killed_women_cum` but extrapolated ([see note](#breakdown-interval))                                   | no       |
| child_famine_cum        | cumulative children killed by starvation ([context](/updates/gaza-daily-famine-aid-aug-23))                     | yes      |
| famine_cum              | cumulative adults & children killed by starvation ([context](/updates/gaza-daily-famine-aid-aug-23))            | yes      |
| aid_seeker_killed_cum   | cumulative people killed at or en route to aid distribution ([context](/updates/gaza-daily-famine-aid-aug-23))  | yes      |
| aid_seeker_injured_cum  | cumulative people injured at or en route to aid distribution ([context](/updates/gaza-daily-famine-aid-aug-23)) | yes      |
...
| civdef_killed_cum       | cumulative number of emergency services killed to date ([see note](#breakdown-interval))                        | yes      |
| ext_civdef_killed_cum   | same as `civdef_killed_cum` but extrapolated ([see note](#breakdown-interval))                                  | no       |
| med_killed_cum          | cumulative number of medical personnel killed to the report date ([see note](#breakdown-interval))              | yes      |
| ext_med_killed_cum      | same as `med_martyed_cum` but extrapolated ([see note](#breakdown-interval))                                    | no       |
| press_killed_cum        | cumulative number of journalists killed to the report date ([see note](#breakdown-interval))                    | yes      |
| ext_press_killed_cum    | same as `press_martyed_cum` but extrapolated ([see note](#breakdown-interval))                                  | no       |
...
Since October 2023, Gaza officials were reporting specific breakdowns for people killed on a semi-regular basis as part of a larger statistical release by the Gaza Media Office. The following fields are examples of these kinds of breakdowns:
...
In 2024, the GMO reported these breakdowns on a weekly basis, and in August 2024 began reporting them about every 2 weeks. In January 2025 this form of reporting was reduced to about every month. On the other hand, the Ministry of Health has begun issuing more frequent names lists (see our [Killed in Gaza dataset](/docs/killed-in-gaza)) which provide more granular demographic information.

The abovenoted fields will only have values in this dataset when we have a report on the given date. Otherwise, the latest reported value will show for subsequent days in the corresponding `ext_` fields. It's important to keep in mind this reduced update frequency when using the extrapolated fields.
